...
# Price retrieval with Token Metrics databse
def get_price_data(coin_symbol):
    ...
    confi_fise: ConfigParser file with SQL database credentials

    return
    -------
    The dataframe containing all the price data for the coin.
    '''
    cred = _pd.read_table('/Users/paul/Documents/Token Metrics/credentials/credentials.txt', sep=',')
    cred = cred.to_dict('records')[0]

    conn = MySQLdb.connect(host=cred['host'], user=cred['user'], passwd=cred['passwd'], db='tokenmetrics')

    query = f"SELECT ico_id, ico_symbol, date, open, close, low, high, volume\
             FROM ico_price_daily_summaries\
             WHERE currency = 'USD' AND ico_symbol = '{coin_symbol}'"

    prices = _pd.read_sql_query(query, conn)
    conn.close()
    if len(prices) == 0:
        print('No price data for this coin.')
        return None
    prices['date'] = _pd.to_datetime(prices.date)

    prices = prices.dropna()
    return prices.sort_values('date').reset_index(drop=True)


def get_stock_data(ticker, hourly=False):

    if hourly:
        prices = _yfinance.Ticker(ticker).history(period="730d", interval='1h')
    else:
        prices = _yfinance.Ticker(ticker).history(period="max")

    if len(prices) == 0:
        print('No price data for this stock.')
        return None

    prices = prices.reset_index(drop=False)
    prices.columns = prices.columns.str.lower()
    prices['ico_id'] = 0
    prices['ico_symbol'] = ticker
    prices.volume = prices.volume.astype(float)
    return prices[['ico_id', 'ico_symbol', 'date', 'open', 'close', 'low', 'high', 'volume']]


def get_trends(prices, days=1):
    '''
    compute the actual real trends of the price data (momentum)
    days specifies how many days in the future we look at to estimate momentum
    '''
    def trend(row):
        if row.close_next >= row.close:
            return 'bullish'
        elif row.close_next < row.close:
            return 'bearish'
        else:
            return "none"

    prices_copy = prices.copy()
    prices_copy['close_next'] = prices_copy.close.shift(-days)
    return prices_copy.apply(trend, axis=1)
...
def plotly_signals(signals, width=1200, height=700, notebook=True):
    trace_price = _go.Scatter(x=signals.date, y=signals.close, mode='lines', hovertext='Close: ' + signals.close.astype(str),
                              name='close', marker=dict(color='grey'))

    trace_bullish = _go.Scatter(x=signals[signals.signal == 1].date, y=signals[signals.signal == 1].close, mode='markers',
                                marker=dict(color='lime', size=7), name='bullish')

    trace_bearish = _go.Scatter(x=signals[signals.signal == -1].date, y=signals[signals.signal == -1].close, mode='markers',
                                marker=dict(color='red', size=7), name='bearish')

    data = [trace_price, trace_bullish, trace_bearish]

    bullish_annotations = [dict(x=xi, y=yi, text='Bullish', showarrow=False, align="center", bgcolor='green', yshift=20, font=dict(color='white'), opacity=.8)
                           for xi, yi in zip(signals[signals.signal == 1].date, signals[signals.signal == 1].close)]

    bearish_annotations = [dict(x=xi, y=yi, text='Bearish', showarrow=False, align="center", bgcolor='red', yshift=-20, font=dict(color='white'), opacity=.8)
                           for xi, yi in zip(signals[signals.signal == -1].date, signals[signals.signal == -1].close)]

    annotations = bearish_annotations + bullish_annotations
    ...
    if notebook is True:
        layout.update(dict(title="Technical indicators Signals"))
...
def plot_pnl(signals, figsize=(17, 10)):
    '''
    Plotting pnl of strategy vs holding.
    '''
    # Start the dataframe after the first buy signal
    signals = signals[signals.index >= signals[signals.signal == 1].index[0]].copy()

    # create a IN/OUT of market column
    signals['market'] = signals.signal.replace(to_replace=0, method='ffill')

    # creating a strategy pct change column to compute strategy ROI
    signals['pct_change_strategy'] = _np.where((signals.market == 1) & (signals.signal != 1) | (signals.signal == -1), signals.close.pct_change(), 0)

    # computing strategy ROI
    signals['strategy_roi'] = (signals.pct_change_strategy + 1).cumprod().fillna(1)
    signals.drop(columns=['pct_change_strategy'], inplace=True)

    # computing hold ROI
    signals['hold_roi'] = (signals.close.pct_change() + 1).cumprod().fillna(1)

    # plotting
    ax = signals.plot(x='date', y='strategy_roi', title='Profit and Loss', label='strategy', color='olive', legend=True)
    signals.plot(x='date', y='hold_roi', label='holding', figsize=figsize, color='silver', legend=True, ax=ax)
    _plt.show()
...
def plot_trends_mama(coin_symbol, btc=None, days=None, figsize=(17, 10)):
    '''
    Plotting prices with MAMA moving averages and SAR trends.
    BTC allows to plot the BTC mama on the same graph
    '''
    # _plt.style.use('fivethirtyeight')
    # _plt.style.use("dark_background")
    # _plt.rcParams['grid.color'] = '#3c4f6e'

    prices = get_price_data(coin_symbol=coin_symbol)
    mama_df = mama(prices=prices, days=days)

    # Not enough price data
    if mama_df is None:
        return None

    trends_df = sar(prices=prices, days=days)
    volume_df = prices[['date', 'volume']].dropna().copy()

    # Plotting main price line
    ax = mama_df.plot(x='date', y='close', figsize=figsize, linewidth=1, label=f'{coin_symbol}', color='slategray', zorder=4)

    # Moving averages cloud - bullish
    ax.fill_between(mama_df.date.values, mama_df['mama'].values, mama_df['fama'].values,
                    color='lime', alpha=.4, where=mama_df['mama'] > mama_df['fama'], label='bullish', zorder=3)

    # Moving averages cloud - bearish
    ax.fill_between(mama_df.date.values, mama_df['mama'].values, mama_df['fama'].values,
                    color='crimson', alpha=.4, where=mama_df['mama'] < mama_df['fama'], label='bearish', zorder=3)

    # Plotting the trend indicator (sar)
    # Intersection of dates to plot
    trends_df = trends_df[trends_df.date.isin(mama_df.date)].copy()

    # Creating a trend feature to color the points properly
    # trends_df['sar_lag'] = trends_df['sar'].shift()
    # trends_df.fillna(method='bfill', inplace=True)
    # trends_df['trend'] = _np.sign(trends_df.sar - trends_df.sar_lag) >= 0
    # trends_df['trend'] = trends_df['trend'].shift(-1)
    # trends_df.drop('sar_lag', axis=1, inplace=True)

    trends_df['sar_color'] = _np.sign(trends_df.sar - trends_df.close) <= 0

    # Plotting the trends points
    _sns.scatterplot(x=trends_df.date.values, y=trends_df.sar.values, ax=ax, s=5, hue=trends_df.sar_color.values,
                     palette={False: 'red', True: 'lime'}, legend=False, zorder=3)

    # Creating a new dataframe only for signals
    mama_signals_1 = mama_df[mama_df.signal == 1].copy()
    mama_signals_min1 = mama_df[mama_df.signal == -1].copy()

    # Annotating signals for better visibility - Bearish signals
    ax.scatter(mama_signals_min1.date.values, mama_signals_min1.close.values, color='Red', s=50, zorder=500)
    for i in range(len(mama_signals_min1)):
        ax.annotate('Bearish', xy=(mama_signals_min1.date.values[i], mama_signals_min1.close.values[i]),
                    textcoords='offset points', xytext=(0, 13), zorder=500, color='white',
                    bbox=dict(boxstyle="Round", fc="red", alpha=.8), horizontalalignment='center', verticalalignment='bottom')

    # Annotating signals for better visibility - Bullish signals
    ax.scatter(mama_signals_1.date.values, mama_signals_1.close.values, color='lime', s=50, zorder=500)
    for i in range(len(mama_signals_1)):
        ax.annotate('Bullish', xy=(mama_signals_1.date.values[i], mama_signals_1.close.values[i]),
                    textcoords='offset points', xytext=(0, 13), zorder=500, color='white',
                    bbox=dict(boxstyle="Round", fc="green", alpha=.8), horizontalalignment='center', verticalalignment='bottom')

    # if btc then plot btc prices
    if btc is True and coin_symbol != 'BTC':

        prices_btc = get_price_data('BTC')
        mama_df_btc = mama(prices=prices_btc)
        mama_df_btc = mama_df_btc[mama_df_btc.date.isin(mama_df.date)]
        trends_df_btc = sar(prices=prices_btc)
        trends_df_btc = trends_df_btc[trends_df_btc.date.isin(trends_df.date)]

        ax2 = ax.twinx()
        # Plotting main price line
        mama_df_btc.plot(x='date', y='close', figsize=figsize, linewidth=1, label='BTC', color='gray', ax=ax2, zorder=1, alpha=.6)

        # Creating a new dataframe only for signals
        mama_signals_1_btc = mama_df_btc[mama_df_btc.signal == 1].copy()
        mama_signals_min1_btc = mama_df_btc[mama_df_btc.signal == -1].copy()

        # Annotating signals for better visibility - Bearish signals
        ax2.scatter(mama_signals_min1_btc.date.values, mama_signals_min1_btc.close.values, color='gray', s=50, zorder=2)
        for i in range(len(mama_signals_min1_btc)):
            ax2.annotate('Bearish', xy=(mama_signals_min1_btc.date.values[i], mama_signals_min1_btc.close.values[i]),
                         textcoords='offset points', xytext=(0, 13), zorder=2,
                         bbox=dict(boxstyle="Round", fc="gray", alpha=.3), horizontalalignment='center', verticalalignment='bottom')

        # Annotating signals for better visibility - Bullish signals
        ax2.scatter(mama_signals_1_btc.date.values, mama_signals_1_btc.close.values, color='gray', s=50, zorder=2)
        for i in range(len(mama_signals_1_btc)):
            ax2.annotate('Bullish', xy=(mama_signals_1_btc.date.values[i], mama_signals_1_btc.close.values[i]),
                         textcoords='offset points', xytext=(0, 13), zorder=2,
                         bbox=dict(boxstyle="Round", fc="gray", alpha=.3), horizontalalignment='center', verticalalignment='bottom')
        ax2.legend(loc='best')

    # Plotting volume if not BTC
    else:
        # Intersetion of dates to plot
        volume_df = volume_df[volume_df.date.isin(mama_df.date)].copy()

        # Creating a twin axis for the volume plot
        ax_bis = ax.twinx()
        ax_bis.fill_between(volume_df.date, volume_df.volume, color='gray', alpha=.4)
        ax_bis.set_ylim(0, volume_df.volume.max() * 5)
        ax_bis.axis('off')

    # Titles and legend
    ax.legend(loc='best')
    _plt.title('Trends Indicator')
    _plt.show()
